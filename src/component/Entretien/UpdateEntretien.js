...
class UpdateEntretien extends Component {
  ...
  validationForm(){
    ...
    if(Object.keys(managerEntretien).length !== 0 && Object.keys(salarie).length !== 0){
      ...
      this.setState({
        message: "Veuillez sélectionner un salarie et un manager",
        ifError: true
      });
      return false
    }
  ...
  saveInterview(e) {
    ...
    entretienService.update(this.state.currentInterview)
        .then((resp) => {
          this.setState({
            message: "Modification de l'entretien bien prise en compte ! Redirection vers la liste des entretiens.",
            ifError: false
        });
        window.setTimeout(() => {this.props.history.push('/entretiens/liste')}, 5000)
        })
        .catch((e) => {
          this.setState({
            message: e,
